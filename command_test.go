...
func TestJoin_TwoFilePositionals(t *testing.T) {
	...
	lines, err := testable.TestLines(
		command.Join(command.JoinFs{Fs: fs}, "left.txt", "right.txt"),
		"ignored\n",
	)
	if err != nil {
		t.Fatal(err)
...
func TestJoin_FileNotFoundPropagates(t *testing.T) {
	fs := afero.NewMemMapFs()
	_, err := testable.TestLines(command.Join(command.JoinFs{Fs: fs}, "missing.txt"), "")
	if err == nil {
...
func TestJoin_SecondPositionalNotFoundPropagates(t *testing.T) {
	...
	_, err := testable.TestLines(
		command.Join(command.JoinFs{Fs: fs}, "left.txt", "missing.txt"),
		"",
	)
	if err == nil {
...
func TestJoin_NoSecondInputYieldsNothing(t *testing.T) {
	fs := afero.NewMemMapFs()
	write(t, fs, "left.txt", "a 1\nb 2\n")
	lines, err := testable.TestLines(command.Join(command.JoinFs{Fs: fs}, "left.txt"), "")
	if err != nil {
		t.Fatal(err)
	}
	if len(lines) != 0 {
